...
export default function SearchBox({
  ...
  const handleInputChange = (e:any) => {
    ...
    const filteredSuggestions = searchArray?.filter((item:any) =>
  ...
  const handleSearchUsingSuggestion = (suggestion:any) => {
    setQuery(suggestion);
    onSearch(suggestion);
  };

  const highlightMatchedCharacters = (text:any, query:any) => {
  ...
  return (
    ...
    >
      ...
      <input
        ...
        onFocus={() => handleFocus()}
        ref={inputRef}
        className="w-full rounded-full focus:outline-none py-3 "
      />
      {query && (
        <button
          onClick={clearQuery}
      ...
      {isFocused && refineQuery && limitedSuggestions?.length > 0 && (
        <div className=" bg-white absolute w-full top-16 text-left shadow rounded-lg  p-4 overflow-auto ">
          <ul>
            {limitedSuggestions.map((suggestion:any, index:any) => (
              <li
                key={index}
                onClick={() => {
